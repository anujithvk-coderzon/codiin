 ...
 * This array drives the home page grid, the footer, the sitemap and the
 * 404 page — change the order here and it changes everywhere.
 */
 ...
 * slug rather than by array slice so reordering PROGRAMS cannot silently
 * move a program into the wrong group.
 */
const DATA_SLUGS: ProgramSlug[] = [
  "data-analytics",
  "data-engineering",
  "data-science",
  "agentic-ai",
...
export const DATA_PROGRAMS = PROGRAMS.filter((p) =>
  DATA_SLUGS.includes(p.slug),
);
export const DEV_PROGRAMS = PROGRAMS.filter(
  (p) => !DATA_SLUGS.includes(p.slug),
);
