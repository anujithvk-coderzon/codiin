...
/** applicationEndDate is a plain "YYYY-MM-DD" string, not a timestamp. Parsed
 *  without a zone it lands on UTC midnight, so it is formatted in UTC too —
 *  otherwise the day could read one earlier for anyone behind UTC. */
const fmtDayOnly = (ymd: string) =>
  ymd
        ...
        <ul className="space-y-3">
          {events.map((event) => {
            ...
            return (
              <li
                key={event.id}
                className="grid grid-cols-[5rem_1fr] gap-x-3 gap-y-3 rounded-xl border border-zinc-200 bg-white p-3 shadow-sm transition hover:border-zinc-300 hover:shadow-md sm:grid-cols-[10rem_1fr] sm:gap-x-4 sm:gap-y-2 sm:p-4 lg:grid-cols-[14rem_1fr]"
              >
                ...
                <div className="col-span-2 min-w-0 sm:col-span-1 sm:col-start-2">
                  <dl className="grid gap-x-5 gap-y-2 text-xs sm:grid-cols-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <dd className="mt-0.5 font-mono tabular-nums text-zinc-700">
                        {fmtDayOnly(event.applicationEndDate)}
                      </dd>
                    </div>
                    <div className="min-w-0">
                      <dt className="uppercase tracking-wider text-zinc-400">
